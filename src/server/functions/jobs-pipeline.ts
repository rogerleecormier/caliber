...
export const getSavedPipelineSearches = createServerFn({ method: "GET" }).handler(async (_, { request }) => {
  const user = await resolveSessionUser(request);
  if (!user) throw new Error("Not authenticated");
  return listSavedSearches(user.id);
});

export const getPipelineCronInfo = createServerFn({ method: "GET" }).handler(async (_, { request }) => {
  const user = await resolveSessionUser(request);
  if (!user) throw new Error("Not authenticated");
