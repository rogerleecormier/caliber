...
export const getSavedLinkedinSearches = createServerFn({ method: "GET" }).handler(async (_data, ctx) => {
  const user = await resolveSessionUser((ctx as any)?.request);
  if (!user) throw new Error("Not authenticated");
  return listSavedLinkedinSearches(user.id);
});

export const getLinkedinCronInfo = createServerFn({ method: "GET" }).handler(async (_data, ctx) => {
  const user = await resolveSessionUser((ctx as any)?.request);
  if (!user) throw new Error("Not authenticated");
